...
.setThumbnail()
.addField(`Ovanakovic Bot | Teşekkürler\n **Beni sunucunuza eklediğiniz için teşekkür ederim**`)
.addField(`Ovanakovic Bot | Nasıl Kullanılır?\n **Beni kullanmak için \`${akardiyan.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Ovanakovic Bot | Linkler\n **\`${akardiyan.prefix}davet\` yazmanız yeterlidir**`)
.setFooter(`Ovanakovic | Gelişmiş Türkçe Bot | 2021`)
.setTimestamp();
...
const player = new Player(client, akardiyan.youtube_api);
...
`⚡️ Botu Davet Et | o+davet`,
`😁 Bizi Tercih Ettiğiniz İçin Teşekkürler`,
`🌟 Prefixi Ayarla | o+prefix`
]
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!777498134828810272>`) return msg.channel.send(`Bunu mu arıyorsun ?\nPrefix => \`${prefix}\``);
});
...
.setDescription(`❌ | <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Ovanakovic Bot | Log Sistemi")
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`**${message.author} Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`**${newMessage.author} Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`**${message.author} Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`**${newMessage.author} Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("**Selam!** `" + member.user.username + "`**o+kayıtol yazarak kayıt olabilirsin!**");
